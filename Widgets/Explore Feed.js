...
const createErrorText = (widget, sortBy) => {
...
const createSmallPostItem = async (post, feedContainerStack) => {
    let { NAME_FONT_SIZE, POST_CONTENT_SIZE } = SETTINGS.SMALL;
...
const createMediumPostItem = async (post, feedContainerStack) => {
    let { NAME_FONT_SIZE, POST_CONTENT_SIZE } = SETTINGS.LARGE;
    let postStack = feedContainerStack.addStack();
    postStack.layoutHorizontally();
...
const getExploreFeed = async (sortBy, limit) => {
    let explore_feed = `{
    ...
    if (res.data && res.data.explorePublications !== null) {
        publicationsData = res.data.explorePublications.items;
...
const createSmallWidget = async (widget, sortBy) => {
    let publicationsData = await getExploreFeed(sortBy, 2);
    if (publicationsData == null) {
        createErrorText(widget);
        return widget;
    }
    let { PADDING, POST_CONTENT_SIZE, TITLE_FONT_SIZE, HANDLE_FONT_SIZE } = SETTINGS.MEDIUM;
    ...
    let title = titleStack.addText(`${createWidgetTitle(sortBy)}`);
...
const createMediumWidget = async (widget, sortBy) => {
    let publicationsData = await getExploreFeed(sortBy, 2);
    if (publicationsData == null) {
        createErrorText(widget);
        return widget;
    ...
    let titleStack = headerStack.addStack();
    titleStack.addSpacer();
    let title = titleStack.addText("Explore Feed" + `  |  ${createWidgetTitle(sortBy)}`);
    title.font = Font.boldRoundedSystemFont(TITLE_FONT_SIZE);
    titleStack.addSpacer();
    let feedContainerStack = mainStack.addStack();
...
const createLargeWidget = async (widget, sortBy) => {
    let publicationsData = await getExploreFeed(sortBy, 4);
    if (publicationsData == null) {
        createErrorText(widget);
        return widget;
    }
    let { PADDING, POST_CONTENT_SIZE, TITLE_FONT_SIZE, HANDLE_FONT_SIZE } = SETTINGS.LARGE;
    widget.setPadding(PADDING, PADDING, PADDING, PADDING);
    let mainStack = widget.addStack();
    mainStack.layoutVertically();
    let headerStack = mainStack.addStack();
    ...
    title.font = Font.boldRoundedSystemFont(TITLE_FONT_SIZE);
    titleStack.addSpacer();
    let sortByStack = headerStack.addStack();
    sortByStack.addSpacer();
    let sortByText = sortByStack.addText(`${createWidgetTitle(sortBy)}`);
    sortByText.font = Font.boldRoundedSystemFont(HANDLE_FONT_SIZE);
    sortByStack.addSpacer();
    let feedContainerStack = mainStack.addStack();
    feedContainerStack.layoutVertically();
...
const createWidgetTitle = (sortBy) => {
    if (sortBy) {
        sortBy = sortBy.toLowerCase().split('_').join(' ');
    }
    switch (sortBy) {
        case 'top commented':
            sortBy = 'Top Commented';
            break;
        case 'top collected':
            sortBy = 'Top Collected';
            break;
        case 'top mirrored':
            sortBy = 'Top Mirrored';
            break;
        case 'latest':
            sortBy = 'Latest';
            break;
        case 'curated profiles':
            sortBy = 'Curated Profiles';
            break;
        default:
            sortBy = '';
            break;
...
const createWidget = async (sortBy) => {
    if (sortBy) {
        sortBy = sortBy.toUpperCase().split(' ').join('_');
    }
    switch (sortBy) {
        case 'TOP_COMMENTED':
        ...
        case 'LATEST':
            sortBy = 'LATEST';
            break;
        case 'CURATED_PROFILES':
            sortBy = 'CURATED_PROFILES';
            break;
        default:
            sortBy = 'CURATED_PROFILES';
            break;
    ...
    switch (widgetSize) {
        case "small":
            await createSmallWidget(widget, sortBy);
            break;
        case 'medium':
            await createMediumWidget(widget, sortBy);
            break;
        case 'large':
            await createLargeWidget(widget, sortBy);
            break;
    }
    return widget;
};
